...
# Mostra um grafico de linha com as avaliaçoes de cada usuário em cada filme
def similarity_plot(prefs, pessoa1, pessoa2)
...
def getRecommendations(prefs,person,coef_viz=0.0,similarity=sim_pearson):
  ...
  for other in prefs:
    ...
    # ignorar notas zero ou menores
    if sim<=coef_viz: continue
...
# Plotar o Gráfico de Preferências para todos os usuários

# configura tamanho da figura
plt.figure(figsize=(10, 5))

# itera pegando o valor das chaves do dicionário, que no caso são os nomes dos usuários
for user in avaliacoes.keys(): 
    # acessa os valores dos subdicionários a partir da chave recuperada
    plt.plot(['Filme ' + str(f + 1) for f in range(len(avaliacoes[user]))], avaliacoes[user].values(), label=user)
    plt.legend()
